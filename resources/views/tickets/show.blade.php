<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 py-8 printable-wrapper">
        ...
        <div class="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm mb-8 no-print flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-2.5 shrink-0">
                <!-- PNG Download Button -->
                <button 
                    onclick="downloadSinglePNG(event)" 
                    type="button"
                    ...
                    title="Simpan tiket langsung sebagai gambar PNG ke Galeri/Downloads"
                >
        ...
        <div class="flex justify-center">
            ...
            <div id="ticket-card-stub" class="ticket-card-stub bg-white border-2 border-[#ffb200] relative w-full max-w-md flex flex-col justify-between box-border shadow-md">
                ...
                <div class="p-6 space-y-5 bg-white flex-grow">
                    ...
                    <div class="p-4 rounded-lg bg-slate-50 border border-slate-200 flex items-center justify-between">
                        <div>
                            <span class="text-[10px] text-slate-400 uppercase tracking-wider block font-bold">Nomor Kursi</span>
                            <span class="font-heading font-black text-xl text-slate-700 tracking-tight">Menunggu Admin</span>
                        </div>
                        <div class="text-right">
                            <span class="text-[10px] text-slate-400 uppercase tracking-wider block font-bold">Status Kursi</span>
                            <span class="font-extrabold text-[10px] text-slate-500 px-3 py-1 rounded bg-white border border-slate-200 inline-block mt-0.5">
                                Akan Ditentukan Panitia
                            </span>
                        </div>
                    </div>
                    @endif
    ...
    <script>
        async function downloadSinglePNG(evt) {
            const card = document.getElementById('ticket-card-stub');
            if (!card) return;

            const btn = evt ? evt.currentTarget : document.querySelector('button[onclick*="downloadSinglePNG"]');
            const originalText = btn ? btn.innerHTML : '';
            if (btn) {
                btn.innerHTML = '<span>Mengunduh PNG...</span>';
                btn.disabled = true;
            }
            ...
            try {
                ...
                link.download = 'E-Ticket-{{ $ticket->ticket_code }}.png';
